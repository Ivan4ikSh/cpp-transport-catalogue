...
namespace transport {
    ...
    void Catalogue::AddBus(const std::string_view& bus_number, const std::vector<std::string_view>& stops_sv, const bool is_circle) {
        std::vector<Stop*> stops = SyncStops(bus_number, stops_sv);
    ...
    Bus* Catalogue::FindBus(std::string_view bus_number) const {
    ...
    std::vector<Stop*> Catalogue::SyncStops(const std::string_view bus_number, const std::vector<std::string_view> stops_sv) {
        std::vector<Stop*> stops;
        for (const std::string_view& stop : stops_sv) {
            Stop* found = FindStop(stop);
            if (found != nullptr) {
